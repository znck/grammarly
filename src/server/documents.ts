...
export interface GrammarlyDocument {
  ...
  findSynonyms(
    offsetStart: number,
    word: string
  ): Promise<Grammarly.TokenMeaning[]>;
}
...
export class DocumentNotFoundException extends Error {
  constructor(
    ...
    message: string = 'DocumentNotFound'
  ) {
    super(message);
  }
}
...
async function getOrCreateGrammarlyDocument(document: TextDocument) {
  if (!grammarlyDocuments.has(document.uri)) {
    grammarlyDocuments.set(
      document.uri,
      await createGrammarlyDocument(document)
    );
  }
...
async function createGrammarlyDocument(document: TextDocument) {
  ...
  const instance: GrammarlyDocument = {
    ...
    findSynonyms: async (offsetStart: number, word: string) =>
      word in instance.synonyms
        ? instance.synonyms[word]
        : (await host.synonyms(offsetStart, word)).synonyms.meanings,
  };

  function getIgnoredNodeTypes(language: string) {
    const ignoreDiagnosticsIn = settings.diagnostics[`[${language}]`] || {
      ignore: [],
    };
  ...
  function isInIgnoredRegion(alert: Grammarly.Alert) {
    ...
    return (
      nodeTypes.size > 0 &&
      getIgnoredNodeTypes(document.languageId).some(nodeType =>
  ...
  host
    ...
    .on(Grammarly.Action.FEEDBACK, result => {
      ...
      sendCustomEventToClient(connection, Grammarly.Action.FEEDBACK, [
        document.uri,
        result,
      ]);
    })
    .on(Grammarly.Action.FINISHED, result => {
      ...
      sendDiagnostics(document);
      sendCustomEventToClient(connection, Grammarly.Action.FINISHED, [
        document.uri,
        result,
      ]);
    });
    ...
    if (e.contentChanges.length && grammarlyDocuments.has(uri)) {
      ...
      if (change.range) {
        ...
        Object.values(grammarly.alerts).forEach(alert => {
          ...
          if (
            offsetStart <= alert.highlightBegin &&
            alert.highlightBegin <= offsetEnd
          ) {
