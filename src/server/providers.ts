...
export async function onCodeAction(
  ...
  { range, textDocument, context }: CodeActionParams
) {
    ...
    .forEach(alert => {
      if (alert) {
        ...
        alert.replacements.map(replacement =>
          actions.push(createGrammarlyFix(alert, replacement, document))
        );
        ...
        if (isSpellingAlert(alert)) {
          ...
          if (isWorkspace)
            actions.push(
              createAddToDictionaryFix(document, alert, 'workspace')
            );
          if (isAuthenticated)
            actions.push(
              createAddToDictionaryFix(document, alert, 'Grammarly')
            );
        }
      }
    });
  ...
  if (word && /^[a-z]+$/.test(word)) {
    const synonyms = await grammarly.findSynonyms(
    ...
    synonyms.forEach(meaning => {
      meaning.synonyms.forEach(replacement => {
        actions.push(
          createGrammarlySynonymFix(word, replacement, document, range)
        );
      });
    });
  }
...
export async function onHover({ position, textDocument }: HoverParams) {
  ...
  alerts.sort(
    (a, b) =>
      a.highlightEnd - a.highlightBegin - (b.highlightEnd - b.highlightBegin)
  );
