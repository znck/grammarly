...
import { Grammarly, GrammarlyDocument } from '@/server/grammarly';
...
interface DiagnosticWithPosition extends Diagnostic, Grammarly.OT.TextRange {
  id: Grammarly.Alert.Id;
  rev: Grammarly.Revision;
}
...
export class GrammarlyDiagnosticsService implements Registerable {
  private LOGGER = new Logger(GrammarlyDiagnosticsService.name);
  private diagnostics = new Map<string, Map<Grammarly.Alert.Id, DiagnosticWithPosition[]>>();
  private histories = new Map<string, Array<{ rev: Grammarly.Revision; changes: Grammarly.OT.TextChange[] }>>();
  ...
  public register() {
    ...
    this.connection.onRequest(GrammarlyLanguageServer.Feature.stop, (ref) => {
      ...
      this.documents.get(ref.uri)?.detachHost();
    });
        ...
        .forEach((alert, index) => {
          ...
          if (alert.title) {
            ...
            hover.contents.push(
              toMarkdown(
                `### ${alert.title}`,
                index === 0 ? (hasFixes ? `🎉 Automated fix available.` : config.cta) : '',
                alert.explanation,
    ...
    this.connection.onCodeAction(async ({ textDocument, context }) => {
      ...
      if (diagnostics.length === 1 && document?.host) {
        ...
        const alert = document.host.getAlert(diagnostic.code as Grammarly.Alert.Id);

        if (alert && alert.replacements.length) {
          const range = Grammarly.OT.TextRange.rebase(
  ...
  private setupDiagnostics(document: GrammarlyDocument) {
    ...
    document.host!.on(Grammarly.ResponseKind.SUBMIT_OT, (message) => {
      const history = this.histories.get(document.uri);
    ...
    document.host!.on(Grammarly.ResponseKind.FINISHED, () => {
      this.sendDiagnostics(document);
    });
  ...
  private rebaseWithChanges(
    document: GrammarlyDocument,
    diagnostic: DiagnosticWithPosition,
    changes: Grammarly.OT.TextChange[]
  ) {
    const range = Grammarly.OT.TextRange.rebase(diagnostic, changes);
  ...
  private rebaseWithHistory(document: GrammarlyDocument, diagnostic: DiagnosticWithPosition, rev: Grammarly.Revision) {
    const changes = this.findChangesSinceRevision(document, rev);

    return this.rebaseWithChanges(document, diagnostic, changes);
  }

  private findChangesSinceRevision(document: GrammarlyDocument, rev: Grammarly.Revision) {
    return this.findHistorySinceRevision(document, rev)
      .map((history) => history.changes)
      .flat();
  }

  private findHistorySinceRevision(document: GrammarlyDocument, rev: Grammarly.Revision, inclusive = false) {
    const history = this.histories.get(document.uri);
    if (history) {
  ...
  private toDiagnostics(alert: Grammarly.Message.Alert, document: GrammarlyDocument): DiagnosticWithPosition[] {
...
function getAlertSeverity(alert: Grammarly.Alert): DiagnosticSeverity {
  if (alert.impact === Grammarly.Alert.Impact.Critical) return DiagnosticSeverity.Error;
