...
export class CodeActionService implements Registerable {
  ...
  register(): Disposable {
    ...
    this.#connection.onCodeAction(
      async ({ textDocument, context }): Promise<CodeAction[]> => {
        ...
        return await Promise.all(
          context.diagnostics
            ...
            .flatMap(({ suggestion, diagnostic }) => {
              ...
              return suggestion.replacements.map(
                async (replacement, index): Promise<CodeAction> => {
                        ...
                        [uri]: [
                          {
                            ...
                            newText: edit.content.ops.map((op) => op.insert).join(''),
                          },
                        ],
                      },
                    },
